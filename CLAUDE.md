...
- **New button = think about a shortcut and UX**: whenever a new button/toggle is added to the player, decide whether it *should* have a keyboard shortcut (anything the user flips mid-practice with a guitar in hand almost certainly should) and whether it *can* (pick a free key — grep the `switch (key)` in `AlphaTabPlayer.jsx` and `KeyboardShortcutsModal.jsx`; prefer a mnemonic letter; don't collide with browser or alphaTab bindings). If you add one, do all four: `case` in the `keydown` switch (via a function ref, see below), the `(K)` hint in the button's `title`, an entry in `KeyboardShortcutsModal.jsx` (`SHORTCUTS`), and the list in `README.md` if it enumerates shortcuts. Also look at the user experience of the button itself: clear on/off state, a `title` that explains what happens, placement next to related controls, and no focus left on the button after a mouse click (see the click-blur rule above).
...
- **Metronome only while playing**: clicks come solely from alphaTab's `midiEventsPlayed` (event type `242`) → sync with playback. The click is **scheduled**, not played on arrival: at a `playbackSpeed` other than 1 alphaTab reports an event up to ~150 ms before it is audible, so `clickDelayFromTicks(event.tick - at.tickPosition, modifiedTempo)` converts the tick lead into a Web Audio `when`. Without it the first beat after a loop wrap landed right (the wrap seeks exactly) while the rest rushed to the song's original tempo — measured 1009 ms / 653 ms around the wrap instead of 833 ms at 72 BPM. `modifiedTempoRef` (from `playerPositionChanged`) is the real playing tempo, tempo automations and BPM slider included. When the song is paused/stopped the metronome is silent even with the toggle on (the earlier standalone Web Audio scheduler was removed deliberately — don't reintroduce it). The only clicks outside playback are the loop count-in below.
...
- **Count-in**: `requestPlayPause` (Space + Play button) plays one bar of Web Audio clicks before starting playback whenever the `1·2·3·4` toggle is on (`C` shortcut) — with or without the loop; a second press during the countdown cancels it. With the loop on, alphaTab's own `isLooping` is turned **off** (`applyLoopRange`/`toggleCountIn`) so the synth stops cleanly at the range end and resets to the range start; `playerFinished` then runs `startCountIn(bar, fromLoopEnd=true)` again, which calls **`stop()` before `play()`** — every loop iteration gets its own count-in without leaking the first note of the next pass. That `stop()` is load-bearing, not cosmetic: with `play()` alone alphaTab's synth drifts a little more with each restart (MIDI events start outrunning the audio, so the metronome speeds up) and around the third pass playback wedges for good — the worker stops feeding samples and the position freezes while the state still says Playing. Measured: without `stop()` the third pass ran the clicks at ~450 ms instead of 833 ms and then froze; with it, 5+ passes stay at 16 690 ± 10 ms. alphaTab's own looping (`isLooping = true`, count-in off) never had the problem, so only this restart path needs it.
